FROM nginx/unit:1.24.0-minimal as builder
...
RUN apt update && apt install -y ca-certificates apt-transport-https \
    ...
    && apt update && apt install -y php7.4 php7.4-mysql php7.4-curl php7.4-dev libphp7.4-embed
...
RUN apt update && apt install -y ca-certificates apt-transport-https \
    && curl -o /etc/apt/trusted.gpg.d/php.gpg https://packages.sury.org/php/apt.gpg \
    && echo "deb https://packages.sury.org/php/ buster main" | tee /etc/apt/sources.list.d/php.list \
    && apt update && apt install -y php7.4-common php7.4-mysql php7.4-curl libphp7.4-embed
